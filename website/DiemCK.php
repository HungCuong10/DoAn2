...
// Hiển thị danh sách
    $sql = $conn -> query("SELECT ct.MSSV, sv.HoVaTen, sv.Lop, ct.DiemGK, ct.DiemCK, lh.TenLop 
...
<form class="user" method="POST" action ="DiemGK.php">
    <div class="card shadow mb-4">
        ...
        <?php if($Seleted == true):?>
                        ...
                        <table class="table table-bordered" style = "text-align: center;" id="dataTable" width="100%" cellspacing="0">
                            ...
                            <?php
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type = "hidden" name = "YC" value = "<?php echo $TT['MaYC'];?>">
                                        <?php 
                                        $Lop = $_POST['LopHP'];
                                        $DSYC = $conn -> query("SELECT * FROM dsyeucau WHERE MaLopHP = '$Lop' AND MaGV = '$username'");
                                        
                                        $TT = $DSYC->fetch_assoc();
                                        if($TT['TrangThaiYC'] == 1):
                                        ?>
                                        <input type = "number" name = "CuoiKi[]" value = "<?php echo $rows['DiemCK'];?>" min = '0.0' max= '10.0' style = 'width: 55px;'>
                                        <?php else: ?>
                                            <input type = "number" name = "CuoiKi[]" value = "<?php echo $rows['DiemCK'];?>" <?php if($rows['DiemGK'] != null){echo 'readonly';} ?> min = '0.0' max= '10.0' style = 'width: 55px;'>
                                     </td>
                                    <?php endif; ?> 
                                    <td>
                                        <input type = "hidden" name = "GiuaKi[]" value = "<?php echo $rows['DiemGK'];?>">
                                        <?php  
                                            
                                            $DiemCK = $rows['DiemCK'] * 0.6;
                                            echo $DiemCK;
                                        ?>
            ...
            <input type = "submit" class='btn btn-primary' name ="GuiYC" value = "Gửi yêu cầu sửa điểm" style = " margin-left:35px">
        <?php endif; ?>
        </div>
    </div>
</from>       
...
// Nút lưu
   ...
   {       
        ...
        for($i = 0; $i < $length; $i++)
        {
            ...
            $a_LopHP = $MaLHP[$i];
            $conn ->query("UPDATE chitietmh SET DiemCK='$a_Diem', DIemTB = '$a_DiemTB' WHERE MSSV = '$a_MSSV' AND MaLopHP = '$a_LopHP'");                
        }
         echo"<script> alert('Lưu điểm thành công')</script>";  
         if($YC) 
         {
            $conn -> query(" DELETE FROM `dsyeucau` WHERE MaYC = '$YC' ");
         }
